    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <form action="{{ route('teacher.sub_chapters.update', $subChapter) }}" method="POST" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" class="w-full rounded-xl border-gray-200 focus:border-[#30b37f] focus:ring focus:ring-[#30b37f] focus:ring-opacity-20 transition-shadow @error('title') border-red-500 @enderror" value="{{ old('title', $subChapter->title) }}" required>
                            @error('title')
                        ...
                        <div>
                            ...
                            <input type="number" name="order_number" id="order_number" class="w-1/3 rounded-xl border-gray-200 focus:border-[#30b37f] focus:ring focus:ring-[#30b37f] focus:ring-opacity-20 transition-shadow @error('order_number') border-red-500 @enderror" value="{{ old('order_number', $subChapter->order_number) }}">
                            @error('order_number')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
