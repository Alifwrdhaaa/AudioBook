        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-xl">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <div class="mt-2 space-x-4">
                                <label class="inline-flex items-center">
                                    <input type="radio" class="rounded-full border-gray-300 text-[#44936d] shadow-sm focus:ring-[#44936d]" name="gender" value="Laki-laki" {{ old('gender', $teacher->gender) == 'Laki-laki' ? 'checked' : '' }} required>
                                    <span class="ml-2">Laki-laki</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" class="rounded-full border-gray-300 text-[#44936d] shadow-sm focus:ring-[#44936d]" name="gender" value="Perempuan" {{ old('gender', $teacher->gender) == 'Perempuan' ? 'checked' : '' }} required>
                        ...
                        <div>
                            ...
                            <div class="mt-2 grid grid-cols-3 gap-2">
                                @php
                                    $selectedClasses = old('classes', $teacher->taughtClasses->pluck('id')->toArray());
                                @endphp
                                @foreach($classes as $schoolClass)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" class="rounded border-gray-300 text-[#44936d] shadow-sm focus:ring-[#44936d]" name="classes[]" value="{{ $schoolClass->id }}" {{ in_array($schoolClass->id, $selectedClasses) ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            @if($teacher->profile_photo)
                                <img src="{{ Storage::url($teacher->profile_photo) }}" class="w-16 h-16 rounded-full mb-2 object-cover">
                            @endif
